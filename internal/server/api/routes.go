...
func Routes() []Route {
	return []Route{
		...
		// --- Vehicles ---------------------------------------------
		{
			...
			Summary:          "List every non-deleted vehicle in the fleet.",
			Tags:             []string{tagFleet},
			Auth:             AuthPublic, // TODO: swap to AuthSession once sessions land
			Response:         VehicleListResponse{},
			ResponseStatuses: []int{http.StatusOK},
			handler:          bindHandler((*Server).handleVehicleList),
		},
		{
			...
			Tags:             []string{tagFleet},
			Auth:             AuthPublic,
			Request:          VehicleCreateInput{},
			Response:         VehicleResponse{},
			ResponseStatuses: []int{http.StatusCreated},
			handler:          bindHandler((*Server).handleVehicleCreate),
		},
		{
			Method:           http.MethodGet,
			Path:             "/v1/vehicles/{id}",
			OperationID:      "getVehicle",
			Summary:          "Fetch one vehicle by id.",
			Tags:             []string{tagFleet},
			Auth:             AuthPublic,
			Response:         VehicleResponse{},
			ResponseStatuses: []int{http.StatusOK},
			handler:          bindHandler((*Server).handleVehicleGet),
		},
		{
			Method:           http.MethodPatch,
			Path:             "/v1/vehicles/{id}",
			OperationID:      "updateVehicle",
			Summary:          "Apply a sparse patch to a vehicle.",
			Tags:             []string{tagFleet},
			Auth:             AuthPublic,
			Request:          VehicleUpdateInput{},
			Response:         VehicleResponse{},
			ResponseStatuses: []int{http.StatusOK},
			handler:          bindHandler((*Server).handleVehicleUpdate),
		},
		{
			Method:           http.MethodDelete,
			Path:             "/v1/vehicles/{id}",
			OperationID:      "deleteVehicle",
			Summary:          "Soft-delete a vehicle. Stamps deleted_at; row remains for audit.",
			Tags:             []string{tagFleet},
			Auth:             AuthPublic,
			ResponseStatuses: []int{http.StatusNoContent},
			handler:          bindHandler((*Server).handleVehicleDelete),
		},
		{
			Method:           http.MethodPut,
			Path:             "/v1/vehicles/{id}/photo",
			OperationID:      "putVehiclePhoto",
			Summary:          "Upload or replace the primary photo. Body is raw image bytes; Content-Type identifies the image format.",
			Tags:             []string{tagFleet},
			Auth:             AuthPublic,
			Request:          openAPIRawBody{},
			Response:         VehicleResponse{},
			ResponseStatuses: []int{http.StatusOK},
			handler:          bindHandler((*Server).handleVehiclePhotoPut),
		},
		{
			Method:           http.MethodGet,
			Path:             "/v1/vehicles/{id}/photo",
			OperationID:      "getVehiclePhoto",
			Summary:          "Stream the primary photo bytes with an appropriate image/* Content-Type.",
			Tags:             []string{tagFleet},
			Auth:             AuthPublic,
			Response:         openAPIRawBody{},
			ResponseStatuses: []int{http.StatusOK},
			handler:          bindHandler((*Server).handleVehiclePhotoGet),
		},
		{
			Method:           http.MethodDelete,
			Path:             "/v1/vehicles/{id}/photo",
			OperationID:      "deleteVehiclePhoto",
			Summary:          "Remove the primary photo. Vehicle itself is unaffected.",
			Tags:             []string{tagFleet},
			Auth:             AuthPublic,
			ResponseStatuses: []int{http.StatusNoContent},
			handler:          bindHandler((*Server).handleVehiclePhotoDelete),
		},
	}
}
